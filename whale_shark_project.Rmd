...
p1 <- ggplot() +
  theme_bw() +
  geom_point(
    data = sharks %>% dplyr::filter(id == "203637"),
    aes(x = lon, y = lat)
  )
...
Since the SSM model needs at least 3 points to fit, there are 8 sharks with less than 3 observations, which will be excluded from our further analysis.
...
52 sharks remaining in the sharks_ssm dataframe.
...
For the sharks that have \>= 3 observations, there are 15 observations of NA date, 15 observations of inf date.
...
2 duplicated entries found, will be removed.
...
In sharks_ssm_clean, all 52 sharks have at least three valid observations with valid timestamps and locations.
...
There are 52 sharks (12144 entries) remaining.
...
Two individuals (157790 and 172246) did not converge.
Their observation histories were examined in relation to temporal gaps.
...
After excluding non-converged fits, 50 sharks remained for further analysis.
...
For this shark (id =203646), the shark has only 9 observations in 2019, with a gap of almost one year, followed by the observation almost one year, where the one year in between have huge uncertainties according to the graph.
...
All 52 shark tracks were visualized on a spatial shapefile:
...
diagnostic_ids <- c("132071", "203646", "164968")
...
Note: Since 175953 and 203646 failed to converge in MPM in later steps, their diagnostics are checked

```{r}
# Additional SSM diagnostics for individuals that later failed MPM convergence
diagnostic_ids_MPM_failed <- c("175953", "203646")
...
In contrast, the shark with id = 203646 shows signs of non-stationarity in the SSM diagnostics.
The residuals display a strong upward trend at early timestamps before stabilizing around zero later, resulting in an asymmetric residual pattern over time.
The ACF plots also suggest subtle temporal structure, with differences between the x and y components.
While the SSM converges, these features likely lead to unstable estimation of movement persistence, explaining the failure of MPM convergence for this individual.
...
After excluding individuals with fewer than 20 predicted locations, 35 sharks were retained for MPM fitting.
Among these, the model did not converge for two individuals (175953 and 203646), which were excluded from subsequent analyses.
...
To highlight individual-level movement geometry, we additionally present maps zoomed to the spatial extent of each track.
